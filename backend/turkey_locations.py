...
TURKEY_DISTRICTS = {
    ...
    68: ["Ağaçören", "Aksaray", "Eskil", "Gülağaç", "Güzelyurt", "Ortaköy", "Sarıyahşi", "Sultanhanı"],
...
def get_districts_by_city(city_id: int):
    """Bir ile ait ilçeleri döndür"""
    return TURKEY_DISTRICTS.get(city_id, [])

def get_city_by_name(city_name: str):
    """İl adına göre il bilgisini döndür"""
...
def search_cities(query: str):
    """İl ara"""
    query = query.lower()
    return [city for city in TURKEY_CITIES if query in city["name"].lower()]

def search_districts(city_id: int, query: str):
    """İlçe ara"""
    query = query.lower()
    districts = TURKEY_DISTRICTS.get(city_id, [])
    return [d for d in districts if query in d.lower()]
